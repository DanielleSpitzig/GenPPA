% Use wine data as an indicator that it works on experimental data
load Wine

[index1,a1]=find(y==1);          %isolate the first two classes of the experimental data
[index2,a2]=find(y==2);

if length(a1)<=length(a2)                %Find the class that smaller - if the same size take the first class
    int=ceil(length(a2)/2);                    %Find half the size of the other class
